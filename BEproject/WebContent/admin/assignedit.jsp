									 ...
									 <div class="form-group">
												<label class="col-sm-2 control-label col-lg-2" for="inputSuccess">108 Staff</label>
													<div class="col-lg-10">
														...
														<c:forEach items="${sessionScope.staffList}" var="k">
														<c:if test="${i.staffVO.staffId eq k.staffId}">
														<option value="${k.staffId}" selected="selected">${k.firstName} ${k.lastName} ${k.middleName}</option>
														</c:if>
														<c:if test="${i.staffVO.staffId ne k.staffId}">
														<option value="${k.staffId}">${k.firstName} ${k.lastName} ${k.middleName}</option>
														</c:if>
														</c:forEach>
												...
												<label class="col-sm-2 control-label col-lg-2" for="inputSuccess">Assign Area</label>
													<div class="col-lg-10">
														<select class="form-control input-sm m-b-10" name="area">
															...
															<c:forEach items="${sessionScope.areaList}" var="m">
														<c:if test="${i.areaVO.areaId eq m.areaId }">
														<option value="${m.areaId}" selected="selected">${m.area}</option>
														</c:if>
														<c:if test="${i.areaVO.areaId ne m.areaId }">
														<option value="${m.areaId}">${m.area}</option>
														</c:if>
														</c:forEach>
														</select>
													</div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" name="id" value="${i.assignAreaId}">
